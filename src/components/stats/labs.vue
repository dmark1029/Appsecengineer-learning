<template>
  <div>
    <BaseTable
      ...
      color="#fff"
    >
      <template v-slot:body-cell-lab_name="props">
        <q-td :props="props">
          <q-item>
            <q-item-section @click="actionRedirect(props.row)">
              <q-item-label class="ase-roboto portal_color cursor_pointer">
                ...
                >
                  {{ props.row.lab_name }}
                  <q-badge v-if="props.row.challenge" size="xs" color="#12142D">Challenge</q-badge>
                </p>
  ...
  methods: {
    async actionRedirect(row) {
      await this.$router.push(
        `/portal/course-info/lab/${urlSafeBase64Encode(row.course)}}/${urlSafeBase64Encode(row.subject)}}/${urlSafeBase64Encode(row.lab)}}/`
      )
    },
    timeSince(time) {
