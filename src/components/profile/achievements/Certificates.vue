<template>
  ...
  <div class="row" v-else>
    <div v-for="(certificate, index) in certificates" class="q-pa-xs col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-2" :key="index">
      <q-card class="cursor-pointer" @click="showAchievementInfo(certificate.certificateNO)">
        <q-card-section>
          <q-img v-if="certificate.certificateImageLink" contain :src="certificate.certificateImageLink" />
          <q-img v-else contain src="~assets/certificate_logo.png"></q-img>
        </q-card-section>
    ...
    <div v-if="certificates.length === 0 && !profileStore.isLoading" class="column flex full-width items-center">
